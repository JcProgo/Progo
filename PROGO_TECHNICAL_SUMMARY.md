...
- **Paleta "un solo acento" (2026-07-11, reemplazó la paleta oro/teal/coral/violeta anterior):** decisión explícita del usuario — la app debía verse "ultra premium", con fondo negro/carbón + azul del logo en oscuro, fondo blanco + naranja del logo en claro, y el resto de la UI en gris neutro puro (sin tintes cálidos/fríos). Implementado sin tocar lógica ni estructura: `gold`/`teal`/`coral`/`violet` en `DARK_THEME`/`LIGHT_THEME` ahora apuntan al mismo valor de acento a propósito, así que cada sección que ya leía `COLORS.gold`/`.teal`/`.coral`/`.violet` lo recoge automáticamente. Contraste acento-sobre-fondo verificado ≥5:1 en ambos temas (el naranja puro del logo, `#FF6B00`, solo daba ~2.9:1 sobre blanco — el acento funcional usa `#C2410C` en su lugar; el naranja vivo original se dejó solo para el glow decorativo del login). **Consecuencia aceptada explícitamente por el usuario:** el selector de 4 tonos de Hábitos/Notas ahora muestra 4 swatches idénticos (ya no diferencia visualmente), y Trading/Ingresos-Egresos/Ganador-Perdedor perdieron el verde/rojo semántico — ganancia vs. pérdida ahora solo se distingue por el signo del número. Documentado en el comment-header de `App.jsx` líneas ~33-45.
