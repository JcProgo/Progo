...
- **Altura de pantalla:** el wrapper raíz de `App()` y `AuthShell` NO usan `100dvh`/`100vh` directamente. `useViewportHeight()` mide `window.innerHeight` por JS y lo aplica como alto en píxeles, recalculando solo en `resize`/`orientationchange`. **A propósito NO usa `window.visualViewport.height`**: esa API se encoge cuando aparece el teclado en iOS (para eso existe), y si el layout reacciona a eso, la barra inferior "sube" empujada por el teclado en vez de quedarse fija — bug real que pasó y se corrigió (ver §9 historial). `html`/`body`/`#root` (en `index.css`) usan `height: 100%; overflow: hidden;` — ese `overflow:hidden` es lo que evita que la página completa haga scroll nativo (rebote elástico de iOS), dejando el área de contenido de `App()` como el único contenedor real con scroll.
- **`apple-mobile-web-app-status-bar-style: black-translucent` — NUNCA volver a agregarlo.** En iOS 18 esa meta (combinada con `apple-mobile-web-app-capable`) encoge la ventana de la PWA instalada varias decenas de puntos respecto a la pantalla física — se diagnosticó con un overlay de debug en vivo (`screen.height` vs `window.innerHeight` no coincidían). Ya se quitó de `index.html` con un comentario explícito ahí mismo advirtiendo no reintroducirla. El modo standalone lo controla solo el manifest (`display: standalone` + `theme-color` sincronizado con el tema).
...
- **Paleta oro/teal/coral/violeta: probado un "un solo acento" y revertido el mismo día (2026-07-11).** El usuario pidió una paleta "ultra premium" de un solo acento (azul del logo en oscuro, naranja en claro, resto gris neutro puro) — se implementó (colapsando `gold`/`teal`/`coral`/`violet` al mismo valor) y se verificó en vivo que se veía limpio y consistente, aceptando explícitamente perder el verde/rojo semántico de Trading/Ingresos-Egresos/Ganador-Perdedor y que el selector de 4 tonos de Hábitos/Notas quedara con swatches idénticos. Tras verlo desplegado, el usuario decidió que prefería el look anterior con más color ("a mí me gustaba como antes con colores") — pero también dijo que el fondo negro más profundo del intento sí le gustaba ("se veía mejor con negro de fondo"). **Resolución final:** se quedó el fondo neutro nuevo (`ink`/`card`/`elevated`/`border`/`paper`/`muted` más "carbón puro", sin el tinte cálido/frío que tenían antes) combinado con los 4 acentos originales (`gold`/`teal`/`coral`/`violet` distintos de nuevo, no colapsados). Contraste de los 4 acentos re-verificado contra el fondo nuevo (todos ≥4.2:1). Ver comment-header de `App.jsx` líneas ~33-48 para los valores finales. **Lección:** para cambios de paleta grandes, considerar mostrar una vista previa/mockup antes de aplicar a toda la app, ya que "un solo acento" se siente muy distinto en la práctica que en la descripción — y a veces la reacción del usuario separa partes del cambio que sí quería (el fondo) de partes que no (el acento único).
